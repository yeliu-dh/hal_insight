...
def train_model(
    ...
    """
    Train a multi-label model with early stopping, printing train/val loss and macro F1 per epoch.
    
    Args:
        model: nn.Module
        train_loader, val_loader: DataLoader
        criterion: loss function (BCEWithLogitsLoss or FocalLoss)
        device: "cuda" or "cpu"
        n_epochs: max epochs
    ...
    Returns:
        model: trained model (weights at last epoch, best saved separately)
    """
    ...
    for epoch in range(n_epochs):
        ...
        # ----- calculate multi-label F1 -----
        preds_bin = (all_probs >= 0.5).astype(int)
        f1_macro = f1_score(all_labels, preds_bin, average='macro')

        print(f"Epoch {epoch+1}/{n_epochs} | "
              f"Train Loss: {train_loss:.4f} | Val Loss: {val_loss:.4f} | F1_macro: {f1_macro:.4f}")
        
        for c in range(all_labels.shape[1]):
            f1_c = f1_score(all_labels[:, c], preds_bin[:, c], zero_division=0)
...
def tune_thresholds_oof(
    model_fn,          # 返回一个新的 nn.Module 实例
    train_dataset,     # 训练集 Dataset
    device,            # "cuda" or "cpu"
    n_splits=5,        # 内层 OOF 折数
    n_epochs=40,       # 与主训练 epoch 保持一致
    batch_size=32,
    ...
    for fold, (tr_idx, va_idx) in enumerate(kf.split(range(len(train_dataset)))):
        print(f"[OOF] Fold {fold+1}/{n_splits}")

        tr_subset = Subset(train_dataset, tr_idx)
        va_subset = Subset(train_dataset, va_idx)
        ...
        # 新模型
        # 每折 OOF 都需要新 optimizer、每折都新模型
        model = model_fn().to(device)
        optimizer = torch.optim.Adam(model.parameters(), lr=1e-3, weight_decay=1e-4)

        # optimizer = torch.optim.Adam(model.parameters(), lr=lr, weight_decay=weight_decay)
        criterion = torch.nn.BCEWithLogitsLoss()  # 或者你的 FocalLoss
        ...
        labels_fold = []
        with torch.no_grad():
            for batch_X, batch_y in va_loader:
                batch_X = batch_X.to(device)
                logits = model(batch_X)
                probs = torch.sigmoid(logits).cpu().numpy()
                probs_fold.append(probs)
                labels_fold.append(batch_y.numpy())

        oof_probs_list.append(np.vstack(probs_fold))
        oof_labels_list.append(np.vstack(labels_fold))

    # 拼接所有 fold
    oof_probs = np.vstack(oof_probs_list)
    oof_labels = np.vstack(oof_labels_list)

    # ---- 按类调 threshold ----
    best_t_per_class = []
    for c in range(oof_labels.shape[1]):
        best_f1, best_t = 0, 0.5
        for t in np.arange(0.05, 0.80, 0.01):
            preds = (oof_probs[:, c] >= t).astype(int)
            f1 = f1_score(oof_labels[:, c], preds, zero_division=0)
            if f1 > best_f1:
                best_f1, best_t = f1, t
        best_t_per_class.append(best_t)

    best_t_per_class = np.array(best_t_per_class)
    print("[OOF] Best thresholds per class:", best_t_per_class)
    return best_t_per_class
...
def tune_thresholds_oof_for_3class(model_fn, train_dataset, device="cpu",
    ...
    """
    计算只针对前三类的 OOF 阈值（MLP 输出 3 类）。
    Args:
        model_fn: 返回一个新的 nn.Module 实例的函数
        train_dataset: Dataset 对象
        device: "cuda" 或 "cpu"
        n_splits: OOF 折数
        n_epochs: 每折训练轮数
        batch_size: batch size
        sampler: 可选 WeightedRandomSampler
        lr, weight_decay: optimizer 参数
    Returns:
        best_t_overall: numpy array shape (3,) 每类最佳阈值
    """
    import numpy as np
    import torch
    from torch.utils.data import DataLoader, Subset
    ...
    X_full = np.array([x.numpy() for x, y in train_dataset])
    y_full = np.array([y.numpy() for x, y in train_dataset])
    ...
    for fold, (train_idx, val_idx) in enumerate(kf.split(X_full)):
        ...
        train_loader = DataLoader(train_subset, batch_size=batch_size,
                                  sampler=sampler if sampler else None, shuffle=(sampler is None))
        val_loader   = DataLoader(val_subset, batch_size=batch_size, shuffle=False)

        # 新模型
        model = model_fn().to(device)
        optimizer = torch.optim.Adam(model.parameters(), lr=lr, weight_decay=weight_decay)
        criterion = torch.nn.BCEWithLogitsLoss()  # OOF 简单用 BCE
        ...
        # 每类 F1-max 阈值
        best_t_per_class = []
        for c in range(all_labels.shape[1]):  # 3 类
            best_f1, best_t = 0, 0.5
            for t in np.arange(0.05, 0.8, 0.01):
                preds = (all_probs[:, c] >= t).astype(int)
                f1 = f1_score(all_labels[:, c], preds, zero_division=0)
                if f1 > best_f1:
                    best_f1, best_t = f1, t
            best_t_per_class.append(best_t)
        all_fold_t.append(best_t_per_class)

    # 平均每类阈值
    best_t_overall = np.mean(all_fold_t, axis=0)
    print("[OOF] Best thresholds (3 classes):", best_t_overall)
    return best_t_overall
...
def train_multilabel_mlp_3class(X_train, X_val, y_train, y_val,
    ...
    """    
    MLP 只预测前三类 → 避免第 4 类稀疏干扰

    FocalLoss gamma 调低到 1 → 输出概率更合理

    Sample weights 取平均 + clip → 避免少数类被过度放大

    OOF 阈值 加下限 0.3 → 避免 [0.1,0.25] 这种奇怪阈值
    ...
    # 构建模型函数，方便 OOF threshold
    def build_model():
        return MultiLabelMLP(input_dim=X_train.shape[1], hidden=hidden, dropout=dropout, n_classes=3).to(device)
    ...
    criterion = FocalLoss(alpha=0.25, gamma=1)  # 调低 gamma，让概率不要太低
...
def mlp_multilabels(X_train,X_val,y_train, y_val, 
    ...
    #             nn.Linear(hidden_dim, 1),  # axe1
    #             nn.Linear(hidden_dim, 1),  # axe2
    #             nn.Linear(hidden_dim, 1),  # axe3
    #             nn.Linear(hidden_dim, 1),  # axe4
    #         ])
    ...
    # model = MultiHeadMLP(input_dim=X_train.shape[1]).to(device)
    ...
    for epoch in range(n_epochs):
        ...
        #用 K-fold 在训练集内部 生成「每个样本的 out-of-fold 概率」，再用这些概率 + 真实标签选 threshold。
        
        
                
        # ----- Per-class threshold tuning -----
        ## 不应该用val来选择threshold，会导致泄露！！
        #得到最好的阈值
        best_t_per_class = []
...
def load_predict_by_mlp_lr(
    ...
    best_t_overall=None,
    t_lr=0.5,
    input_dim=None,    # 可选：如果传入就用这个
    hidden=512,
    dropout=0.5,
    scaler=None        # 可选：可以传入训练时的 StandardScaler
):
    """
    Load trained MLP + LR models and predict multi-label axes on new data.

    Args:
        df_long: DataFrame, must have "text_emb" and "source_vec"
        mlp_model_path: path to saved MLP checkpoint
        lr_model_path: path to saved LR model (joblib)
        best_t_overall: optional, list/array of thresholds for MLP [4]
        t_lr: threshold for LR (axe4)
        input_dim: optional, input dim of MLP
        hidden, dropout: MLP architecture
        scaler: optional, StandardScaler fitted on training features

    Returns:
        df_long with pred_axe1..pred_axe4 columns
    """
    ...
    # ----------------- prepare features -----------------
    X = np.concatenate([df_long["text_emb"].tolist(), df_long["source_vec"].tolist()], axis=1)
    ...
    mlp_model = MultiLabelMLP(input_dim=input_dim, hidden=hidden, dropout=dropout).to(device)
    ...
    # 如果 best_t_overall 没传，就尝试从 checkpoint 里加载
    if best_t_overall is None and 'best_t_overall' in checkpoint:
        best_t_overall = checkpoint['best_t_overall']
        print(f"[INFO] Loaded best_t_overall from checkpoint: {best_t_overall}")
    ...
    # MLP
    with torch.no_grad():
        probs_mlp = torch.sigmoid(mlp_model(torch.from_numpy(X).float().to(device))).cpu().numpy()
    if best_t_overall is not None:
        preds_mlp = (probs_mlp >= best_t_overall).astype(int)
    else:
        preds_mlp = (probs_mlp >= 0.5).astype(int)  # fallback
    ...
    # ----------------- combine final axes -----------------
    pred_final = np.zeros((n_samples, 4), dtype=int)
    pred_final[:, :3] = preds_mlp[:, :3]
    pred_final[:, 3] = preds_lr  # LR predicts axe4

    # ----------------- evaluate if true labels exist -----------------
    required_cols = ['axe1','axe2','axe3','axe4']
    if all(col in df_long.columns for col in required_cols):
        if not (df_long[required_cols].sum(axis=0) == 0).all():
            from utils.axe_classification import evaluate_model
            y_true = df_long[required_cols].values.astype(np.float32)
            print("[INFO] Evaluating model on provided labels".center(80,'-'))
            evaluate_model(y_true=y_true, y_pred=pred_final)
            print()

    # ----------------- add predictions to df -----------------
    df_long["pred_axe1"] = pred_final[:,0]
    df_long["pred_axe2"] = pred_final[:,1]
    df_long["pred_axe3"] = pred_final[:,2]
    df_long["pred_axe4"] = pred_final[:,3]

    return df_long, probs_mlp, probs_lr
...
def predict_by_mlp(
    ...
    X: np.ndarray [N, D]
    threshold:
        - None        -> 0.5 for all classes
        - float       -> same threshold for all classes
        - array-like  -> per-class threshold, shape [C]
    return:
        dict with keys: probs, preds
    """
    import numpy as np
    import torch
    ...
    if threshold is None:
        ...
        if threshold.ndim == 0:
            threshold = np.full(n_classes, threshold)
...
def load_predict_by_mlp_lr(
    ...
    # ===================== build X =====================
    X = np.concatenate(
        [df_long["text_emb"].tolist(), df_long["source_vec"].tolist()],
        axis=1
    ).astype(np.float32)
    print(f"[INFO] Input X shape: {X.shape}")
    ...
    class MultiLabelMLP(torch.nn.Module):
        def __init__(self, input_dim, hidden, dropout):
            ...
            self.net = torch.nn.Sequential(
                ...
                torch.nn.ReLU(),
                torch.nn.Dropout(dropout),
                torch.nn.Linear(hidden // 2, len(class_names_mlp))
            )

        def forward(self, x):
            return self.net(x)

    checkpoint = torch.load(path_mlp, map_location=device, weights_only=False)
    ...
    best_t = checkpoint.get("best_t_overall", np.full(len(class_names_mlp), 0.5))
    best_t = np.array(best_t[:len(class_names_mlp)])

    # ===================== MLP prediction =====================
    with torch.no_grad():
        logits = mlp_model(torch.from_numpy(X).to(device))
        probs_mlp = torch.sigmoid(logits).cpu().numpy()

    preds_mlp = (probs_mlp >= best_t[None, :]).astype(int)

    # ===================== LR prediction (axe4) =====================
    lr_model = joblib.load(path_lr)

    # ⚠️ 假设 LR 是 pipeline（含 scaler）
    probs_lr = lr_model.predict_proba(X)[:, 1]
    preds_lr = (probs_lr >= t_lr).astype(int)

    # ===================== merge =====================
    n = len(df_long)
    y_pred = np.zeros((n, len(class_names_mlp) + 1), dtype=int)
    y_pred[:, :len(class_names_mlp)] = preds_mlp
    y_pred[:, -1] = preds_lr

    # ===================== attach to df =====================
    for i, name in enumerate(class_names_mlp):
        df_long[f"pred_{name}"] = y_pred[:, i]

    df_long[f"pred_{class_name_lr}"] = y_pred[:, -1]

    # ===================== optional evaluation =====================
    # gt_cols = list(class_names_mlp) + [class_name_lr]
    # if all(col in df_long.columns for col in gt_cols):
    #     y_true = df_long[gt_cols].values.astype(int)
    #     print("\n" + "-" * 30 + " Evaluation " + "-" * 30)
    #     evaluate_model(
    #         y_true=y_true,
    #         y_pred=y_pred,
    ...
    do_eval = (
        all(col in df_long.columns for col in gt_cols)
        and df_long[gt_cols].values.sum() > 0
    )

    if do_eval:
        print("[INFO] Valid ground-truth labels found → go to model evaluation!")
        ...
        print("[INFO] No valid ground-truth labels found → skip evaluation, prediction only.")
...
# def load_predict_by_mlp_lr(df_long, mlp_model_path,
#                 lr_model_path,  
#                 best_t_overall,        
#                 t_lr=0.5):
...
#     X_s = scaler.fit_transform(X)
#     probs_lr = lr_model.predict_proba(X_s)[:,1]
#     pred_lr = (probs_lr >= t_lr).astype(int)#单独一个模型的预测结果
...
#     required_cols = ['axe1', 'axe2', 'axe3', 'axe4']
#     if not all(df_long[required_cols].sum()==0):
#         y_true = df_long[['axe1','axe2','axe3','axe4']].values.astype(np.float32)
#         print("evaluate model's predictions".center(100,'-'))
#         evaluate_model(y_true=y_true, y_pred=pred_final)
#         print()
...
def gather_predicted_axes(df_long_predicted, 
                       ...
                       groupby_col='halId_s'):
    """
    对于每一个id，集合它在不同文本上得到的预测标签，贴回原df的predicted_axe列上
